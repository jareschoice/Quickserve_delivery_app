...
class _WalletPageState extends State<WalletPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final data = await _api.getJson('/vendors/wallet');
      if (!mounted) return;
      setState(() {
        _balance = (data['balance'] as num).toDouble();
        _next = DateTime.tryParse(data['nextWithdrawAt']?.toString() ?? '');
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _withdraw() async {
    try {
      await _api.post('/vendors/wallet/withdraw', {});
      if (!mounted) return;
      ScaffoldMessenger.of(context)
